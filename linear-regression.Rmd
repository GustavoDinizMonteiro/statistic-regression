...
```{r}
# analyzing the participation by gender
data_2006_genner_general = data_2006 %>% 
  group_by(sexo) %>% 
  summarise(num = n())

data_2006_genner_general %>% 
  ggplot(aes(x = sexo, y = num)) +
  geom_col() +
  labs(x='Genero', y='Numero de representantes na eleição')
```
...
## TODO: FIX group by geder and political party
data_2006_genner %>% 
  ggplot(aes(x = (partido), y = num)) +
  geom_col(width = 0.2)
  labs(x='Genero', y='Numero de representantes na eleição')
```
